...
import { LocationAdministrativeContext, getDistrictProfile } from '../data/locationContextData.js';
...
export function analyzeLocationConditionedImpact(
  ...
  if (isIndustrial) {
    ...
    domainConditionEvaluations.economic = {
      ...
      impactClassification: 'Potential Positive',
      positiveImpacts: ['Potential secondary commercial footfall for local services around government administrative hub'],
      negativeImpacts: ['Public capital expenditure from state exchequer'],
      locationFactors: ['Urban commercial service zone'],
      evidence: ['Government administrative expenditure budget'],
  ...
  if (isIndustrial && (isAgriConversion || isEcoSensitive)) {
    ...
    domainConditionEvaluations.policy_compliance = {
      ...
      relevance: 'Directly Relevant',
      impactClassification: 'Potential Positive',
      positiveImpacts: ['Project aligns with government administrative and public works policy guidelines'],
      negativeImpacts: ['Requires standard financial sanction and building plan approvals'],
      locationFactors: ['Government public-use land allocation'],
